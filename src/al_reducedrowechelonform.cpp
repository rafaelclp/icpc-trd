...
int main() {
	const int n = 5, m = 4; VVT a(n);
	double A[n][m]=
		{{16,2,3,13},{5,11,10,8},{9,7,6,12},{4,14,15,1},{13,21,21,13}};
	for (int i = 0; i < n; i++) a[i] = VT(A[i], A[i] + n);

	int rank = rref(a); cout << "Rank: " << rank << endl; // expected: 4
	
	// expected: 1 0 0 1/0 1 0 3/0 0 1 -3/0 0 0 2.78206e-15/0 0 0 3.22398e-15
	cout << "rref: " << endl;
	for (int i = 0; i < 5; i++) {

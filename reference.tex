...
\fancyhead[L,LO]{Federal University of Campina Grande}
...
\author{Federal University of Campina Grande}
...
\SECTION{Cubic function roots}{
    \Csourcefile{\detokenize{al_3degree}}
}

\SECTION{Fibonacci}{
    \Csourcefile{\detokenize{al_fibonacci}}
}

\SECTION{Number theoretic algorithms (modular, Chinese remainder, linear Diophantine)}{
    \Csourcefile{\detokenize{al_euclid}}
}

\SECTION{Primes}{
    \Csourcefile{\detokenize{al_prime}}
}

\SECTION{Reduced row echelon form, matrix rank}{
    \Csourcefile{\detokenize{al_reducedrowechelonform}}
}

\SECTION{Simplex algorithm}{
    \Csourcefile{\detokenize{al_simplex}}
}

\SECTION{Systems of linear equations, matrix inverse, determinant}{
    \Csourcefile{\detokenize{al_gaussjordan}}
}
...
\SECTION{Brent's Algorithm (Cycle detection)}{
    Let $x_0\in S$ be an element of the finite set $S$ and consider a function $f:S\to S$. Define
        ...
        \Csourcefile{\detokenize{cd_brent}}
}
...
\SECTION{Convex--hull}{
    \Csourcefile{\detokenize{al_convexhull}}
}

\SECTION{Miscellaneous geometry}{
    \Csourcefile{\detokenize{al_geometry}}
}

\section{Strings}

\SECTION{Knuth--Morris--Pratt Algorithm (KMP)}{
    \Csourcefile{\detokenize{al_kmp}}
}

\SECTION{Lex--rot}{
    \Csourcefile{\detokenize{al_lexrot}}
}

\SECTION{Longest palindrome (Manacher)}{
    \Csourcefile{\detokenize{al_manacher}}
}

\SECTION{Suffix Array and Longest Common Prefix}{
    \Csourcefile{\detokenize{al_suffixarray}}
}

\SECTION{Z-Algorithm}{
    \Csourcefile{\detokenize{al_zalgorithm}}
}

\section{Graphs}

\SECTION{Articulation points} {
    \Csourcefile{\detokenize{al_artpoints}}
}

\SECTION{Bellman--Ford} {
    \Csourcefile{\detokenize{al_bellmanford}}
}

\SECTION{Eulerian Path} {
    \Csourcefile{\detokenize{al_eulerianpath}}
}

\SECTION{Max bipartite matching} {
    \Csourcefile{\detokenize{al_maxbipartitematching}}
}

\SECTION{Max--flow (Dinic)} {
    \Csourcefile{\detokenize{al_dinic}}
}

\SECTION{Min--cost matching} {
    \Csourcefile{\detokenize{al_mincostmatching}}
}

\SECTION{Min--cost max--flow} {
    \Csourcefile{\detokenize{al_sspdijkstra}}
}

\SECTION{Min--cut} {
    \Csourcefile{\detokenize{al_stoerwagner}}
}

\SECTION{Strongly connected components (Tarjan)} {
    \Csourcefile{\detokenize{al_tarjan}}
}

\SECTION{2-Sat} {
    \Csourcefile{\detokenize{al_2sat}}
}

\SECTION{Tree distance sum} {
    \Csourcefile{\detokenize{al_treedistsum}}
}

\section{Data Structures}

\SECTION{Bigint} {
    \Csourcefile{\detokenize{ds_bigint}}
}

\SECTION{Hashstring} {
    \Csourcefile{\detokenize{ds_hstring}}
}

\SECTION{Lowest Common Ancestor (LCA)} {
    \Csourcefile{\detokenize{ds_lca}}
}

\SECTION{Segment Tree} {
    \Csourcefile{\detokenize{ds_segtree}}
}

\SECTION{Segment Tree (with Lazy Propagation)} {
    \Csourcefile{\detokenize{ds_lsegtree}}
}

\SECTION{Union--Find} {
    \Csourcefile{\detokenize{ds_unionfind}}
}

\section{Miscellaneous and C++ STL reference}

\SECTION{Dates library} {
    \Csourcefile{\detokenize{al_dates}}
}

\SECTION{Josephus problem} {
    \Psourcefile{\detokenize{al_josephus}}
}

\SECTION{STL reference} {
    \Csourcefile{\detokenize{rf_stl}}
}

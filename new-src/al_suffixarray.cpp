...
template<int maxlen> class SuffixArray {
	int n, sz;
...
public:
	int sa[maxlen], invsa[maxlen]; // invsa[sa[x]] == x
	void gen_sa(char* s, int length) { // could be int* s or string& s
		n = length, invsa[n] = -1;
	...
	int lcp[maxlen]; // run gen_sa BEFORE gen_lcp
	void gen_lcp(char* s) { // could be int* s or string& s
...
}; SuffixArray<MAXN> SA; // SA.gen_sa(s, strlen(s)); int* sa = SA.sa;